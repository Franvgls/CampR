...
GetAlk.camp<-function(gr,esp,camp,dns="Porc",plus=8,n.ots=FALSE,AltAlk=FALSE) {
  ...
  if (plus<15) edad<-data.frame(edad[,1:(plus+4)],plus=rowSums(edad[,(plus+5):ncol(edad)]))
  if (!n.ots) edad<-data.frame(edad[,4:3],edad[,5:ncol(edad)]/rowSums(edad[,-c(1:4)])) else edad<-data.frame(edad[,4:3],edad[,5:ncol(edad)])
