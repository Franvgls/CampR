...
CAMPtoHLmolus<-function(camp,dns,inclSpecie=F,quart=T,incl2=F) {
    ...
    ntalls <-as.data.table(DBI::dbGetQuery(ch1,paste0("select * from NTALL",camp," where GRUPO='3'")))
    names(ntalls)<-tolower(names(ntalls))
    ch2 <- DBI::dbConnect(odbc::odbc(), dsn = "Camp")
    on.exit(DBI::dbDisconnect(ch2), add = TRUE)
    especies <-as.data.table(DBI::dbGetQuery(ch2,paste0("select * from ESPECIES where GRUPO='3'")))
...
#    especies<-subset(especies,especies$esp %in% unique(ntalls[ntalls$grupo==3,"esp"]))
    ...
    DB1<-merge(ntallsdumb,as.data.table(DB),by="lance",all.x=T)
