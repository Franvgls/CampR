...
#' 
#' Salida de datos a csv para rellenar los informes de grupo de trabajo, filas con datos ab estratificada (Biomasa y N) y error estándar por subdivisión ICES función para el Demersales Norte (saca 9.a, 8.c Este y Oeste) 
...
#' @seealso {\link{databICESDiv} \link{databEstr} \link{datab}}
#' @export
databICES<-function(gr,esp,camp,dns="Cant",cor.time=TRUE,Nas=FALSE) {
  if (length(camp)>1) {stop("seleccionadas más de una campaña, no se pueden sacar resultados de más de una")}
  if (all(substr(dns,1,4)!="Cant",substr(dns,1,4)!="Cnew")) {stop("Función sólo disponible para Demersales Costa Norte divisiones 9.a, 8.c Este y 8.c Oeste")}
  esp<-format(esp,width=3,justify="r")
  dumb1<-sapply(CV.camp(gr,esp,camp,dns,cor.time=cor.time,excl.sect=c(3:5),Nas=Nas)$sectores[1:2,],t)
  dumb2<-t(CV.camp(gr,esp,camp,dns,cor.time=cor.time,excl.sect=c(1:2),Nas=Nas)$total[1:2,])
  dumb3<-t(CV.camp(gr,esp,camp,dns,cor.time=cor.time,Nas=Nas)$total[1:2,])
  dumb4<-sapply(CV.camp(gr,esp,camp,dns,cor.time=cor.time,ind="n",excl.sect=c(3:5),Nas=Nas)$sectores[1:2,],t)
  dumb5<-t(CV.camp(gr,esp,camp,dns,cor.time=cor.time,ind="n",excl.sect=c(1:2),Nas=Nas)$total[1:2,])
  ...
  colnames(dumb7)<-c("9.aN_Avg","9.aN_SE","8.cW_Avg","8.cW_SE","8.cE_Avg","8.cE_SE","Tot_Avg","Tot_SE")
  dumb7
}
