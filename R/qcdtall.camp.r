...
qcdtall.camp<- function(gr,esp,camp="P12",dns="Pnew",lance,ti=F,legend=T,idi="l",ymax=NA) {
  ...
  ntalls<-sqlQuery(ch1,paste("select lance,peso_gr,peso_m,cate,talla,sexo,numer from NTALL",camp,
                             " where grupo='",gr,"' and esp='",esp,"' and lance='",lance,"'",sep=""))
  odbcClose(ch1)
  ntalls$wgnum<-ntalls$numer*ntalls$peso_gr/ntalls$peso_m
  dtalls1<-tapply(ntalls$wgnum,ntalls[,c("talla","cate")],sum,na.rm=T)
  dtalls2<-tapply(ntalls$numer,ntalls[,c("talla","cate")],sum,na.rm=T)
  dtalls<-data.frame(talla=as.numeric(rownames(dtalls1)),tot=rowSums(dtalls1,na.rm=T))
  ordcat<-tapply(ntalls$talla[ntalls$numer>0],ntalls[ntalls$numer>0,c("cate")],min,na.rm=T)
  ncat<-length(ordcat)
  wghts<-tapply(ntalls$peso_gr/ntalls$peso_m,ntalls$cate,mean,na.rm=T)
  smps<-tapply(ntalls$peso_m,ntalls$cate,mean,na.rm=T)
  dumb<-as.data.frame(c(1:(trunc(max(dtalls[,1])/10)*10+10)))
  names(dumb)<-"talla"
  dtall<-merge(dumb,dtalls,by="talla",all.x=T)
  ...
  for (i in 1:ncol(dtalls2)) {
    ...
    dtall<-merge(dtall,dumb,by="talla",all.x=T)
  }
  dtall[is.na(dtall)]<-0
  leg<-paste("Categ.",names(ordcat)[order(ordcat)])
  ntots<-colSums(dtall,na.rm=T)
  opar<-par(no.readonly=T)
  #  if (length(wghts)>1) par(mfrow=c(2,2))
  par(mgp=c(3,0.7,0.7),cex.axis=.7)
  barplot(dtall$tot,main=buscaesp(gr,esp),font.main=4,space=0,axes=F,ylim=c(0,max(dtall$tot)*1.1))   # names=dtall$talla,
  axis(2,cex.axis=.8,line=0)
  axis(1,at=seq(1,nrow(dtall),by=3)+.5,labels=dtall$talla[seq(1,nrow(dtall),by=3)],cex.axis=.8,line=0)
  box()
  barplot(t(as.matrix(dtall[,3:ncol(dtall)])),add=T,col=2:5,space=0,axes=F)
