...
#' @seealso MeanMaxL.camp {\link{MeanMaxL.camp}}
#' @examples MeanMaxL.camps(gr=1,c(50,42,43,44,45),Nsh[7:27],"Cant")
#' @export
MeanMaxL.camps<- function(gr=1,esps,camps,dns="Cant",lmax=NA,especiales=F,excl.sect=NA,plot=T,es=T,ti=F) {
  ...
  dumb1<-MeanMaxL.camp(gr=gr,esps=esps,camps[1],dns=dns,lmax=lmax,especiales=especiales,excl.sect=excl.sect)
  ...
  for (i in 2:length(camps)) {
    dumb1<-MeanMaxL.camp(gr=gr,esps=esps,camps[i],dns=dns,lmax=lmax,especiales=especiales,excl.sect=excl.sect)
  ...
  if (plot) {
    plot(MeanLmax~year,dumbind,type="o",pch=21,bg="grey",ylim=c(0,ceiling(max(dumbind$MeanLmax,na.rm=T)*1.1)),
