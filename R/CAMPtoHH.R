...
CAMPtoHH<-function(camp,dns,quart=T,incl2=F) {
    ...
    if (substr(dns,1,4)=="Pnew" | substr(dns,1,4)=="Porc") {
       ...
       DB$Warpdia=20
       if(any(nchar(DB$lance)>2)) stop("Lances con más de 2 carácteres, Porcupine no suele tener más de 99 lances, revise datos")
    ...
    HH_north<-data.table::data.table(RecordType="HH",Quarter=DB$quarter,Country="SPA",Ship=DB$barco,Gear=DB$Gear,SweepLngt=DB$malletas,GearExp=-9,DoorType=DB$DoorType,StNo=DB$StNo,HaulNo=DB$lance,Year=DB$year,Month=substr(DB$fecha,4,5),Day=substr(DB$fecha,1,2),TimeShot=DB$TimeShot,Stratum=DB$estrato,HaulDur=round(DB$haul.mins*DB$weight.time),DayNight="D",ShootLat=DB$latitud_l,ShootLong=DB$longitud_l,HaulLat=DB$latitud_v,HaulLong=DB$longitud_v,StatRec=DB$icesrect,Depth=DB$prof_l,HaulVal=ifelse(DB$validez==1,"V","I"),HydroStNo=DB$estn,StdSpecRecCode=1,BycSpecRecCode=0,DataType="R",Netopening=round(DB$abert_v,1),Rigging=-9,Tickler=-9,Distance=round(DB$recorrido),Warplngt=DB$cable,Warpdia=DB$Warpdia,WarpDen=-9,DoorSurface=DB$DoorSurface,DoorWgt=DB$DoorWght,DoorSpread=trunc(DB$dista_p),WingSpread=round(DB$abert_h,1),Bouyancy=-9,KiteDim=-9,WgtGroundRope=-9,TowDir=formatC(DB$rumbo,flag=0,width=3),GroundSpeed=DB$velocidad,SpeedWater=-9,SurCurDir=-9,SurCurSpeed=-9,BotCurDir=-9,BotCurSpeed=-9,WindDir=DB$dir_viento,WindSpeed=DB$vel_viento,SwellDir=-9,SwellHeight=DB$est_mar,SurTemp=-9,BotTemp=DB$temp,SurSal=-9,BotSal=DB$sali,ThermoCline=-9,ThClineDepth=-9)
