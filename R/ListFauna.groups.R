...
#' @export
ListFauna.groups<- function(camp,dns,excl.sect=NA,incl2=FALSE,verbose=TRUE,cor.time=TRUE,kg=TRUE) {
  dumb<-ListFauna.camp(1,camp,dns,excl.sect=excl.sect,incl2=incl2,verbose=verbose,cor.time=cor.time)
    for (i in 2:5) {
      dumb<-rbind(dumb,ListFauna.camp(i,camp,dns,excl.sect,incl2,cor.time = cor.time))
    }
