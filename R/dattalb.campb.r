#' Abundancia estratificada por talla y sexo forzando a que sean en cm. Para datos medidos en mm o .5
#'
...
#' @return Devuelve un data.frame con variables: talla e indeterminados, no
#' @seealso {\link{datos.camp}}
#' @examples dattal.campb("1"," 18","N24","Cant")
#' @export
dattal.campb<- function(gr,esp,camp,dns,cor.time=TRUE,cor.5=FALSE,excl.sect=NA,sex=TRUE,verbose=TRUE) {
  ...
  if (length(esp)>1 | any(esp=="999")) {
    ...
    #if (cor.5 & increm==1 & medida=="mm") stop("La corrección de .5 cm a cm es para medidos en .5 mm, no para medidos en mm")
#    if (length(unique(increm))>1 | length(unique(medida))>1) stop("Seleccionadas especies medidas en distintas unidades (mm y cm o .5 cm) o a la aleta anal")
  ...
  if (length(esp)==1) {
    ...
    if (esp=="999") {
      if (cor.5) stop("No se puede utilizar la corrección de .5/mm a cm para más de una especie")
    }
  }
  if (length(esp)>1) {
    ntalls<-DBI::dbGetQuery(ch1,paste0("select lance,peso_gr,peso_m,talla,sexo,numer from NTALL",camp,
