...
#' @examples mapICESStatRec(1,43,"P21","Porc",plus=8,bw=F)
#' @family mapas
#' @family edades
#' @export
mapICESStatRec<-function(gr,esp,camp,dns="Cant",age=0,plus=3,cor.time=TRUE,ICESrect=TRUE,AltAlk=NA,incl2=FALSE,bw=FALSE,ti=TRUE,plot=T,
  out.dat=FALSE,ind="n",cexleg=1,idi="l",es=TRUE,years=TRUE,escmult=.1,layout=NA,ceros=FALSE,mediahora=2,lattice=TRUE) {
  options(scipen=2)
    ...
    if (lattice) {
		mapdist<-lattice::xyplot(lat~long|name,Result3,layout=layout,xlim=c(-15.5,-10.5),main=titulo,sub=sub,xlab=NULL,ylab=NULL,
          ...
          lattice::panel.xyplot(x,y,cex=ifelse(Result3$numero[subscripts]>0,sqrt((Result3$numero[subscripts])/escala),.35),
		    ...
		    MapHistAge<-maphistage(gr,esp,camp,"Porc",age=0,plus=plus,out.dat=T,plot=F,incl2 = F)
