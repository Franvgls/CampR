...
#' @family PescaWin
#' @export
MapLansHH<-function(fic,dns="Cant",nurows=NA,incl0=FALSE,xlims=NA,ylims=NA,col=2,lwd=2,places=TRUE,Nlans=FALSE,es=T,bw=FALSE,ax=T) {
  ...
  segments(lan$ShootLong,lan$ShootLat,lan$HaulLong,lan$HaulLat,col=col,lwd=lwd)
  if (incl0) segments(lannul$ShootLong,lannul$ShootLat,lannul$HaulLong,lannul$HaulLat,col=2,lwd=2)
