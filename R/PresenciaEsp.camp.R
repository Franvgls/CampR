...
#' @family Series datos
#' @export
PresenciaEsp.camp<- function(gr,esp,dns,tablas=FALSE,redux=TRUE) {
   ...
   if (tablas) data.table(campanas=dumb[grepl("CAMP",dumb)],idCamps=idCamp,faunas=dumb[grepl("FAUNA",dumb)],tallas=dumb[grepl("NTALL",dumb)])
   }
   }
