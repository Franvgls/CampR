...
#' @param mtallas Si T mira también el fichero ntalls, si F sólo mira en el fauna. En el caso de las basuras mucho mejor hacerlo con tallas=F
#' @param redux si T no incluye los datos de las campañas en las que no aparece la especie, sólo los positivos, para ver todas las campañas en directorio mejor {\link{CampsDNS.camp}}
...
#' @family Series datos
#' @export
PresenciaEsp.camp<- function(gr,esp,dns,tablas=FALSE,mtallas=TRUE,redux=TRUE) {
   ...
   if (mtallas) camps.t<-substr(dumb[grepl("NTALL",dumb)],6,8)
   ...
   if (mtallas) tallas$ESP<-as.numeric(as.character(tallas$ESP))
   if (nrow(fauna)==0) {
   ...
   if (length(levels(absp$camp))>0) {
      ...
      if (mtallas) {
         if(sum(!camps.t %in% levels(absp$camp))>0 & !redux) message(paste0("no aparece en las campañas ",paste(camps.f[!camps.t %in% levels(absp$camp)],collapse=", ")))
         if (length(levels(ntalls$camp))>0) {
               ...
               if(sum(!camps.t %in% levels(ntalls$camp))>0 & !redux) message(paste0("no hay datos de tallas en las campañas ",paste(camps.t[!camps.t %in% levels(ntalls$camp)],collapse=", ")))
            }
         else message(paste("No hay información de tallas para",buscaesp(gr,esp)))
      }
   }
   if (tablas & mtallas) data.table(campanas=dumb[grepl("CAMP",dumb)],idCamps=idCamp,faunas=dumb[grepl("FAUNA",dumb)],tallas=dumb[grepl("NTALL",dumb)])
   if (!mtallas) data.table(campanas=dumb[grepl("CAMP",dumb)],idCamps=idCamp,faunas=dumb[grepl("FAUNA",dumb)])
   }
   }
