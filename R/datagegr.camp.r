...
#' @export
datagegr.camp<- function(gr,esp,camp,dns="Pnew",plus=8,mediahora=1,cor.time=T) {
  ...
  lan<-sqlQuery(ch1,paste("select lance,sector,estrato,hora_l,hora_v from LANCE",camp," where validez<>'0'",sep=""))
  ...
  edad<-GetAlk.camp(gr,esp,camp,dns,plus)
  ...
  if (any(sapply(b,length)>0)) { # | any(sapply(bb,length)>0)
    ...
    ch1<-odbcConnect(dns)
    lan<-datlan.camp(camp,dns,redux=F)[,c("lance","latitud_l","latitud_v","longitud_l","longitud_v","sector","estrato","ewl","ewv","weight.time")] 
    #                        from LANCE",camp," where validez='1'",sep=""),errors=F)
    odbcClose(ch1)
    if (all(lan==-1)) {
      ...
      lanedad<-merge(lan,lanedad,by.x="lan",by.y="lance",all.x=T)
