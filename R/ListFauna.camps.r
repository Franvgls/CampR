...
#' @export
ListFauna.camps<- function(gr="1",camps,dns,excl.sect=NA,incl2=F) {
  dumb<-cbind(camp=camps[1],ListFauna.camp(gr,camps[1],dns,excl.sect,incl2))
  if (length(camps)>1) {
    for (i in camps[2:length(camps)]) {
      dumb<-rbind(dumb,cbind(camp=i,ListFauna.camp(gr,i,dns,excl.sect,incl2)))
    }
  }
  dumb[order(as.character(dumb$especie)),]
}
#ListFauna.camps(gr="1",Nsh[25:27],"Cant",excl.sect=NA,incl2=T)