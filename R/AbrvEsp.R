...
#' los spp. cuidado.
#' Avisa si hay códigos duplicados en el resultado de los códigos solicitados, para evitar crear códigos repetidos para distinas especies
...
#'@param unique Evitar duplicados en los códigos, no tiene mucho sentido pero se puede dejar como falso y tener repetidos los códigos
...
AbrvEsp<- function(x,nGen=3,nEsp=4,sep="_",unique=TRUE) {
  ...
  if (length(x1[duplicated(x1)])>0) {message(paste0("Registros repetidos para: ",paste(x[duplicated(x1)],collapse = ", ")))
    x1<-make.names(x1,unique = unique)}
