...
#' Si hay que sacarla por stock o subarea funciona el excl.sect =1 (VIIIc) =c(2:5) (IXa) (Para Demersales)
...
#' @seealso talpes.camp {\link{talpes.camp}}
#' @examples propmat.camp(1,50,"N05",l50=37)
#' @export
propmat.camp<-function(gr=1,esp,camp,dns="Cant",l50=37,b=T,excl.sect=NA) {
  if (length(camp)>1) {stop("seleccionadas más de una campaña, no se pueden sacar resultados de más de una")}
  if (length(esp)>1) {stop("seleccionadas más de una especie, este indicador es monoespec?fico")}
  esp<-format(esp,width=3,justify="r")
  dumb<-dattal.camp(gr,esp,camp,dns,sx=F,excl.sect=excl.sect)
  if (b) {
