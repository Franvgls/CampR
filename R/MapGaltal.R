...
#' @param cexleg Var?a el tama?o de letra de los ejes y del n?mero de la leyenda
#' @param years Si T saca los a?os como nombre de campa?a en los paneles lattice de campa?as
...
#' @examples MapGaltal(1,50,Nsh[21:29],"Cant",1,23,layout=c(3,3),out.dat=T,ceros=F)
#' @export
MapGaltal<-function(gr,esp,camps,dns="Cant",tmin,tmax,ind="n",sx=NA,bw=T,ti=T,plot=T,idi="l",cexleg=1,years=F,layout=NA,out.dat=T,leg=T,ceros=T,escmult=.25) {
  ...
  if (length(esp)>1 | esp=="999") {
    ...
    if (ind=="p") {
      stop("No se pueden calcular capturas en peso de un rango de tallas para m?s de una especie")
      }
    medida<-c("cm")
    }
  ...
  esp<-format(esp,width=3,justify="r")
	...
	dumb<-NULL
	for (i in 1:ndat) {
	  if (!is.null(dattalgr.camp(gr,esp,camps[i],dns,tmin,tmax,sx,ind))) dumb<-rbind(dumb,cbind(dattalgr.camp(gr,esp,camps[i],dns,tmin,tmax,sx,ind),camp=camps[i]))
	}
	if (years) {
    dumbcamp<-dumb
	...
	mapdist<-xyplot(lat~long|camp,dumb,layout=layout,xlim=c(-10.25,-6),main=titulo,xlab=NULL,ylab=NULL,subscripts=T,
		ylim=c(41.82,44.3),aspect=asp,par.strip.text=list(cex=cexleg*.9,font=2),scales=list(alternating=F,tck=c(1,0),cex=cexleg*.7,
		...
		panel=function(x,y,subscripts=subscripts) {
			...
			if (max(dumb$numero[subscripts],na.rm=T)>0) {
				panel.xyplot(rep(-8.2,3),c(43.,42.60,42.20),cex=sqrt((leyenda)/escala),pch=16,col=colo)
				ltext(rep(-8.2,3),c(43.,42.60,42.20),labels=paste(leyenda,ifelse(ind=="p","kg","ind.")),pos=4,offset=1,cex=cexleg*.8
        )
				}
			if (ind=="p") {panel.xyplot(x,y,cex=ifelse(dumb$peso[subscripts]>0,sqrt((dumb$peso[subscripts])/escala),.4),
				pch=ifelse(dumb$peso[subscripts]>0,16,4),col=colo)}
			else {panel.xyplot(x,y,cex=ifelse(dumb$numero[subscripts]>0,sqrt((dumb$numero[subscripts])/escala),.35),
				pch=ifelse(dumb$numero[subscripts]>0,16,4),col=colo)}
			})
