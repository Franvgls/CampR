...
#' @examples
#' ArteParCompV("N23","Cant",Nlans = F,ti=T)
#' ArteParCompV("N23","Cant",Nlans = T,ti=T,lan.cex=2)
#' @family gear
#' @export
ArteParCompV<-function(camp,dns="Cant",incl2=TRUE,es=T,bw=TRUE,ti=TRUE,sub=TRUE,out.dat=FALSE,
  ...
  par(mfcol=c(1,2))
  plot(abert_v~prof,lan,pch=ifelse(Nlans,NA,21),ylim=c(0,hablar::max_(abert_v)*1.1),
       xlim=c(hablar::min_(prof),hablar::max_(prof)*1.1),xlab=paste(ifelse(es,"Prof","Depth"),"(m)"),
       ylab=paste(ifelse(es,"abertura vertical","Net opening"),"(m)"))
  if (Nlans) text(abert_v~prof,lan,label=lance,cex=1*cex.leg,font=2,pos=4)
  if (ti) title(paste(ifelse(es,"Abertura vertical con profundidad","Net opening vs. depth"),"CAMP IEO"))
  if (sub) mtext(paste0(dns," Q",lan$quarter),3,adj=1,font=1,cex=.8)
  plot(Netopening~Depth,landatr,pch=ifelse(Nlans,NA,21),subset = Netopening>0,
       ylim=c(0,hablar::max_(Netopening)*1.1),xlim=c(hablar::min_(Depth),hablar::max_(Depth)*1.1),
       xlab=paste(ifelse(es,"Prof","Depth"),"(m)"),
        ylab=paste(ifelse(es,"Abertura vertical","Net opening"),"(m)"))
  if (Nlans)  text(Netopening~Depth,landatr,label=HaulNo,subset = Netopening>0,pos=4,font=2,cex=1*cex.leg)
  if (ti) title(paste(ifelse(es,"Abertura vertical con profundidad","Net opening vs. depth"),"DATRAS"))
  ...
  if (out.dat) {
    if (out.dat) merge(lan[,c("lance","abert_v")],landatr[,c("HaulNo","Netopening")],by.x="lance",by.y = "HaulNo")
  }
}
