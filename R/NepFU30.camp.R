...
#' @export
NepFU30.camp<-function(camp=camp,dns="Arsa",trimes=4,plot=TRUE,es=FALSE,ti=TRUE,ICESlab=FALSE,ceros=T,leg=T,escmult=.25,cexleg=1,
  ...
  lans_FU30<-datlan.camp(camp,"Arsa",redux=T,incl2=T,incl0 = F)
  #lans_FU30<-rbind(lans_FU30,dplyr::filter(datlan.camp(Nsh,"Arsa",redux=T,incl2=T),c(long>c(-3) & long<c(-2) & lat >c(43) & lat<(44))))
  MapArsa(ICESrect = ICESrect,bw=bw,ICESlab = ICESlab,ICESrectcol = ICESrectcol,FU="FU30",ColFU=ColFU,dens=dens,
