...
#' @param w Es un factor de ponderación para cada uno de los valores de x. Sirve para la función de la librería boot boot(x,strmean,R,stype="f",Porc.map=sector,sector=sector,area=area,camps=camps) 
...
#' @export
strmean.camps<-function(x,sector,area,w=rep(1,length(x)),camps) {
  dummy<-tapply(x*w,data.frame(sector,camps),mean,na.rm=T)
  area.dumb<-tapply(area,data.frame(sector,camps),mean,na.rm=T)
  mean.dumb<-colSums(dummy*area.dumb,na.rm=T)/colSums(area.dumb,na.rm=T)
  mean.dumb
}
