...
NepFU31.camp<-function(camp=camp,dns="Cant",plot=TRUE,es=FALSE,ti=TRUE,ICESlab=TRUE,
                      ICESrectcol=2,ICESrect=TRUE,FU=31,places=TRUE,out.dat=TRUE,bw=FALSE) {
  ...
  MapNort(ICESrect = T,ylims=c(42.5,44.5),xlims=c(-6.5,-1.5),bw=bw)
  title(main=camptoyear(Nsh[i]),line=1.5,sub=paste("FU 31 Nep Catch (n)= ",
                                                   sum(Nep_31[Nep_31$camp==Nsh[i],"numero"])),cex.sub=1.2,cex.main=2)
  points(lat~long,Nep_31,subset=c(peso.gr>0 & camp==Nsh[i]),cex=sqrt(Nep$numero/5),pch=21,col=2,bg=2)
  points(lat~long,Nep_31,subset=c(peso.gr==0 & camp==Nsh[i]),cex=.7,pch=21,col=1,bg=1)
