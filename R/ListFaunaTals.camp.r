...
#' @seealso ListFauna.camp {\link{ListFauna.camp}}
#' @export
ListFaunaTals.camp<- function(gr="1",camp,dns,excl.sect=NA,incl2=T) {
  if (length(camp)>1) {stop("seleccionadas más de una campaña, no se pueden sacar resultados de más de una")}
  require(RODBC)
  ch1<-odbcConnect(dsn=dns)
  odbcSetAutoCommit(ch1, FALSE)
  listsps<-sqlQuery(ch1,paste("select esp,lance from FAUNA",camp," where grupo='",gr,"'",sep=""))
  if (incl2) 
  { lan<-sqlQuery(ch1,paste("select lance,sector,estrato from LANCE",camp," where validez<>'0'",sep=""),errors=F)}
  else lan<-sqlQuery(ch1,paste("select lance,sector,estrato from LANCE",camp," where validez='1'",sep=""),errors=F)
  talls<-sqlQuery(ch1,paste("select lance,esp,talla,numer,peso_m,peso_gr from NTALL",camp," where grupo='",gr,"'",sep=""))
  odbcClose(ch1)
  ...
  #browser()
  dumbres[order(as.character(dumbres[,3]),decreasing=F),]
}
#ListFaunaTals.camp(gr="1","N90","Cant",excl.sect=NA)