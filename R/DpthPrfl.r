...
#' @export
DpthPrfl<-function(gr,esp,camps,dns="Pnew",ind="p",es=T,ti=T,idi="l",xmax=NA,nlans=T,spl=F,brks="Sturges",tabres=T) {
    ...
    dumb<-maphist(gr,esp,camps,dns,plot=F,out.dat=T,ind=ind)
    ...
    else {dumbDatDpth<-hist(rep(dumb$prof,dumb$peso.gr/1000),plot=F,breaks=dumbDpth$breaks)}
  ...
  # DpthPrfl(1, 50, "N08", "Cant",brks = "norte",tabres=T,ind="p")
  # DpthPrfl(1,50,"P08","Pnew",brks="porcupine")
  # DpthPrfl(1,50,"P08","Pnew",brks="porcupine",ti=F)
