...
CampsDNS.camp<- function(dns) {
  ...
  camps<-tibble(tipo="CAMP",ident=substr(dumb[grepl("CAMP",dumb)],5,7))
  camps<-rbind(camps,data.frame(tipo=substr(dumb[!grepl("CAMP",dumb)],1,5),ident=substr(dumb[!grepl("CAMP",dumb)],6,8)))
  camps.c<-tibble(tipo=dumb[grepl("CAMP",dumb)],ident=substr(dumb[grepl("CAMP",dumb)],5,7))
  camps.l<-tibble(tipo=dumb[grepl("LANCE",dumb)],ident=substr(dumb[grepl("LANCE",dumb)],6,8))
  camps.f<-tibble(tipo=dumb[grepl("FAUNA",dumb)],ident=substr(dumb[grepl("FAUNA",dumb)],6,8))
  camps.t<-tibble(tipo=dumb[grepl("NTALL",dumb)],ident=substr(dumb[grepl("NTALL",dumb)],6,8))
  camps.h<-tibble(tipo=dumb[grepl("HIDRO",dumb)],ident=substr(dumb[grepl("HIDRO",dumb)],6,8))
  camps.e<-tibble(tipo=dumb[grepl("EDAD",dumb)],ident=substr(dumb[grepl("EDAD",dumb)],5,7))
  ...
  if (nrow(DBI::dbGetQuery(ch1,paste0("select FECHA from ","LANCE",camps$ident[camps$tipo=="LANCE"][1])))>0){
    m<-lubridate::year(as.Date((DBI::dbGetQuery(ch1,paste0("select FECHA from ","LANCE",camps$ident[camps$tipo=="LANCE"][1])))[1,],format="%Y-%m-%d")) %% 100
    m<-ifelse(m>70,1900+m,2000+m)
    YeCamp<-tibble(camp=paste0("LANCE",camps$ident[camps$tipo=="LANCE"][1][1]),year=m)
  }
  else YeCamp<-tibble(camp=NA,year=NA)
  for (i in 2:length(paste0("LANCE",camps$ident[camps$tipo=="LANCE"]))) {
    if (nrow(DBI::dbGetQuery(ch1,paste0("select FECHA from ","LANCE",camps$ident[camps$tipo=="LANCE"][i])))>0) {
      m<-lubridate::year(as.Date((DBI::dbGetQuery(ch1,paste0("select FECHA from ","LANCE",camps$ident[camps$tipo=="LANCE"][i])))[1,],format="%Y-%m-%d")) %% 100
  ...
  DD<-tibble(NomCamp=nombres,Year=as.numeric(anyos_lan),Camp=camps.c,Lance=camps.l,
                    Fauna=camps.f,Tallas=camps.t,Edad=camps.e,Hidro=camps.h)
