...
#' @param buscfam en vez de buscar el nombre de la especie busca a través de la familia, si T no muestra la lista de especies, sólo la familia, si F no seleccona por familia
...
#  Encoding(especies$ESPECIE)  <- "UTF-8"
  if (buscfam) print(especies[grep(nomb,especies$FAMILIA,ignore.case=T),c(1,3,4,2)])
  else print(especies[grep(nomb,especies$ESPECIE,ignore.case=T),c(1,3,4,2)])
}
