...
#' @param cex Tamaño del texto de las etiquetas de cuadrícula si labs=T
#' @seealso sorteo {\link{sorteo.r}}
#' @export
mapsorteo<-function(x,wmf=F,ax=T,nomfic="porco.emf",lwdl=.5,colo=NA,add=F,labs=F,col=2,cex=.9) {
  require(gstat)
  ...
  if (!add)	mapporco(lwdl=lwdl,cuadr=T,ax=ax,colo=colo,corners=T)
  if (!labs) {
  ...
  else text(x$x,x$y,labels=x$pt,cex=cex,font=2,col=col)
  if (wmf) dev.off()
